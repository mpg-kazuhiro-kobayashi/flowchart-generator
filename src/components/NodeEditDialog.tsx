      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        >
          ...
          <p className="text-white/80 text-sm mt-1">
            ...
            {isStartNode && (
              <span className="ml-2 px-2 py-0.5 bg-green-400 rounded text-xs">開始</span>
            )}
            {isEndNode && (
              <span className="ml-2 px-2 py-0.5 bg-purple-400 rounded text-xs">終了</span>
            )}
            {!isSpecialNode && isRootNode && (
              <span className="ml-2 px-2 py-0.5 bg-blue-400 rounded text-xs">ルートノード</span>
            )}
          </p>
        </div>
        ...
        {edgeConflicts.length > 0 && (
          <div className="px-4 py-3 bg-red-50 border-b border-red-200">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm">
                <p className="font-medium text-red-800">エッジ条件が競合しています</p>
              ...
              {isSpecialNode ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p
                        className={`font-medium ${isStartNode ? "text-green-800" : "text-purple-800"}`}
                      >
                        {isStartNode ? "開始ノード" : "終了ノード"}
                      </p>
                      <p
                        className={`text-sm ${isStartNode ? "text-green-600" : "text-purple-600"}`}
                      >
                        {isStartNode
