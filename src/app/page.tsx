...
import FlowchartRenderer, {
  EdgeClickInfo,
  ConflictingEdgeInfo,
} from "@/components/FlowchartRenderer";
...
import { FlowchartDefinition, CustomNode, FlowchartNode, NodeEntryRule } from "@/types/flowchart";
...
export default function Home() {
  ...
  const { isNodeDialogOpen, selectedSourceNode, openNodeDialog, closeNodeDialog } = dialogState;
  ...
  const handleEdgeClick = useCallback(
    (edgeInfo: EdgeClickInfo) => {
      ...
      const sameFromToEdges = customEdges.filter(
        (e, i) =>
          i <= edgeInfo.edgeIndex && e.from === edgeInfo.fromNodeId && e.to === edgeInfo.toNodeId,
      );
  ...
  const handleUpdateNode = useCallback(
    (nodeId: string, update: NodeUpdateResult) => {
      setCustomNodes((prev) =>
        prev.map((node) => {
  ...
  // EntryRule 追加のハンドラ
  const handleAddEntryRule = useCallback(
    (nodeId: string, rule: Omit<NodeEntryRule, "id">) => {
  ...
  const entryRuleDialogAvailableNodes = useMemo((): FlowchartNode[] => {
    ...
    return customNodes.filter(
      (n) => n.id !== entryRuleDialogInfo.targetNodeId && n.nodeType !== "end",
    );
  ...
  const handleEntryRuleDialogUpdate = useCallback(
  ...
  const handleEntryRuleDialogDelete = useCallback(
        ...
        sourceNode={
          selectedSourceNode
            ? {
                ...
                questionCategory: customNodes.find((n) => n.id === selectedSourceNode.id)
                  ?.questionCategory,
