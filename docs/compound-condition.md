...
5. システムが自動で：
   ...
   - 選択したノード → 状態ノードへのエッジを追加（点線、複合条件ラベル付き）
   - 状態ノード → 接続先ノードへのエッジを追加
```

### 重要: 経路ベースの条件判定

複合条件が選択可能になるのは、**選択したノードに到達するまでの経路上に**設問ノード（SA/MA/NA）が2つ以上含まれる場合のみです。フローチャート全体のノード数ではなく、経路上のノードで判定します。

詳細は `compound-condition-path-analysis.md` を参照してください。
